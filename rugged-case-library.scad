...
hinge_screw_case_v_offset = hinge_screw_lid_v_offset + seal_height + seal_depth - 1;    // Offset of hinge screw from the top of the case
hinge_screw_distance = hinge_screw_lid_v_offset + hinge_screw_case_v_offset;

// Lock variables
// TODO: n_locks
lock_screw_length = 25;  // Screw length for the sides. Length of the case lock screw is lock_screw_length - 2 * wall_thickness - length of the head of this third screw. So generally it is 25 - 2*3 - 3 = 16 mm.
lock_tightness = 0.015;  // 0 is a perfect fit for the lock sides. If too loose, increase this value.
lock_hinge_angle1 = 15;  // Angle between the lock hinge screw and the case screw. Increasing this value makes the lock more "clicky".
lock_hinge_angle2 = 10;  // Angle between the case wall and the lock hinge. If too small, the lock won't be easily accessible.
...
lock_hinge_offset = lock_screw_total_v_offset;  // Vertical distance between the case screw and the hinging screw of the lock. NOT total euclidean distance!
...
module ruggedCase() {
    difference() {
        union() {
            ...
            // Hinges
            translate([-(hinge_screw_length-hinge_wall_thickness)/2, outer_depth/2, outer_height-hinge_screw_case_v_offset])
        ...
        if (seal_enable) translate([0,0,outer_height]) {
            ...
            difference() {
                ...
                hull() {
                    ...
                    translate([0,0,-1])
                    roundedCube([inner_width+2*o,inner_depth+2*o,2], radius=inner_radius+o, center=true);
                }
            };
        };
        ...
        difference() {
            difference() {
                ...
                hull() {
                    translate([0,0,bottom_rounding/2])
                    roundedCube([outer_width-2*bottom_rounding, outer_depth-2*bottom_rounding, bottom_rounding], radius=outer_radius-bottom_rounding, center=true);
        ...
        mirror([1,0,0]) rotate([0,0,-90]) linear_extrude(0.6) text(case_text, size=text_size, halign="center", valign="center", font=text_font);
    };
}
...
module ruggedLid() {
    ...
    difference() {
        union() {
            ...
            difference() {
                hull() {
                    ...
                    translate([0,0,-0.1])
                    roundedCube([inner_width+o,inner_depth+o,0.2], radius=inner_radius+o/2, center=true);
                };
                hull() {
                    translate([0,0,1+seal_depth])
                    roundedCube([inner_width+2*seal_wall,inner_depth+2*seal_wall,2],
            ...
            // Hinges
            translate([-(hinge_screw_length-hinge_wall_thickness)/2, outer_depth/2, outer_height-hinge_screw_lid_v_offset])
        ...
        *translate([0,0,inner_height/2+wall_thickness])
        ...
        rotate([0,0,90]) mirror([1,0,0]) linear_extrude(0.6) text("Machiavelli", size=7.5, halign="center", valign="center", font="Liberation Sans:style=Bold Italic");
    };
};
...
module seal() {
    ...
    difference() {
        roundedCube([inner_width+2*seal_wall+2*seal_width, inner_depth+2*seal_wall+2*seal_width, seal_height],
                    radius=inner_radius+seal_wall+seal_width, center=true);
        roundedCube([inner_width+2*seal_wall, inner_depth+2*seal_wall, seal_height],
                    radius=inner_radius+seal_wall, center=true);
    };
};
